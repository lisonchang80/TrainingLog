...
- **一般動作區**：目標 per `(Template name, 週期, 強度)` 三元組獨立（原 `(Template name, Program, 副標籤)`），跟著週期化變化
...
- **(a) 跟著週期化變的「主菜」**（深蹲、臥推、硬舉等大動作）：12-15RM → 10-12RM → 8-10RM → 6-8RM 漸進，每個 (週期, 強度) 三元組（原 (Program, 副標籤)）的目標必然不同
