...
剩下未 land：**D0** spike（3 spike 結果寫 commit body，hard blocker for D6 + D7 wire-in + D9 wire-in）、**D4** Xcode Watch target、**D5** Watch HK lifecycle、**D6**、**D7 wire-in**、**D8**、**D9 wire-in**、**D10**-**D18**、**D22**、**D23**、**D24 wire-in**、**D25**、**D27**。D21 視為已落地（label 在 main 上是 `slice-13d/D5`），未來不再 implement D21。D7 / D9 / D24 各自的「partial」commit 落了純邏輯子集，wire-in 子集留待 connectivity.ts D3 bridge / settings repository 落地時補完。
