...
|---|---|---|---|---|---|
| 2026-05-27 evening | D-chain 推進策略：純 TS commit 可亂序提前 ship | （2026-05-26 28-commit chain 落地段）forward 接 D4 → D5 → ... 順序進行；隱含每條 D-commit 都要前置依賴先 land | **純 TS / 純 logic / 無 native dep 的 D-commit 可亂序提前 ship**；UI wire-in 留待 connectivity.ts 落地時補。當天 D19 (iPhone live mirror reducer) + D20 (set-modified per-field LWW) 兩條按此原則提前做完、跨越 D6-D18 多條未 land 的 native bridge / Watch UI commit | 用戶當天 6 backlog 候選都做完後接著做純 TS 工作；D0 spike 需實機 (user 排時段)、D4 Watch Xcode target 卡 D0、D6/D7 卡 connectivity.ts 卡 D0；不卡的純 TS commit 留著「等順序」是浪費 | D20 = `2e3b13d`、D19 = `791a0ed`；本 commit = D26 docs(adr) |
...
剩下未 land：**D0** spike（3 spike 結果寫 commit body，hard blocker for D6/D7）、**D4** Xcode Watch target、**D5** Watch HK lifecycle、**D6**-**D18**、**D22**-**D27**。D21 視為已落地（label 在 main 上是 `slice-13d/D5`），未來不再 implement D21。

**D-chain 推進策略 update（2026-05-27 evening）**：原規約「forward 接 D4 → D5 → ... 順序進行」翻盤 — 純 TS / 純 logic / 無 native dep 的 commit 可亂序提前 ship（見 2026-05-27 翻盤 ledger row）。D19 + D20 兩條被優先做完是這個原則的首批落地；下一個純 TS 候選包含 D7 reducer 子集（end-session reconciliation 5-sec timer 純邏輯）+ D24 settings readout 邏輯。Watch 端（D8-D17 + D22）+ 卡 D0 spike 的 D6/D7 native bridge 留實機開工時做。
