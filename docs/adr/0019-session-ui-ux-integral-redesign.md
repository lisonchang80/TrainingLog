...
- `SessionStatsPanel` variant logic 不變 — Phase A 已正確 branch on `is_watch_tracked`、Phase B 只是讓真 Watch-tracked session 自然 trigger 5-tile（dev toggle 移除後條件僅剩 session.healthkit_workout_uuid !== null）
...
1. **OS permission dialog 是 1-shot**：`initHealthKit()` 第一次 call 才彈、之後 same app 不會再彈。User 改主意必須去 Settings.app → 隱私 → 健康 → TrainingLog 自己 toggle scope。
