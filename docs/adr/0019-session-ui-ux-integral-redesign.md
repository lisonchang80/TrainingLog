...
- **(c) F1** — Timer 倒到 0 → 震動 + 短音 + ~~chip 消失~~ → **modal auto-dismiss**（slice 10d X1 修訂；短音仍 deferred to slice 13）（不彈紅警告 / 不擋互動）（**2026-05-25 Slice 13 Phase A 拍板**：短音採 0.3s sine 440Hz beep `assets/sounds/rest-timer-done.wav` + `expo-audio` (SDK 54+) 觸發，於既有 `Haptics.notificationAsync(Success)` 同一處 fire — 詳見 § Slice 13 Phase A Amendment）
...
|---|---|---|---|---|---|
| 2026-05-27 evening | D-chain 推進策略：純 TS commit 可亂序提前 ship | （2026-05-26 28-commit chain 落地段）forward 接 D4 → D5 → ... 順序進行；隱含每條 D-commit 都要前置依賴先 land | **純 TS / 純 logic / 無 native dep 的 D-commit 可亂序提前 ship**；UI wire-in 留待 connectivity.ts 落地時補。當天首批落地 4 條 pure-logic commits（D20 LWW / D19 live mirror / D7 partial reconciliation reducer / D9 partial handshake builders），跨越 D6-D18 多條未 land 的 native bridge / Watch UI commit；額外加 D26 partial ADR doc sweep（兩次）+ 把模式萃成 `ship-partial-pure-logic` project skill | 用戶當天 6 backlog 候選都做完後接著做純 TS 工作；D0 spike 需實機 (user 排時段)、D4 Watch Xcode target 卡 D0、D6 / D7 wire-in / D9 wire-in 卡 connectivity.ts 卡 D0；不卡的純 TS commit 留著「等順序」是浪費 | D20 = `2e3b13d`、D19 = `791a0ed`、D26 partial = `54b8f9d`、D7 partial = `9a29ef6`、skill = `faf09f5`、D9 partial = `c350b5c`；本 commit = D26 docs(adr) 二次補刀 |
...
**Shipped 進度（per main HEAD 2026-05-27 evening、跟下表 chain slot 對齊）**
...
| `slice-13d/D7-partial` | `9a29ef6` | 2026-05-27 12:13 | **D7 partial** | end-session 5-sec reconciliation reducer（純 TS + Clock-injected `tick` action，無 setTimeout；wire-in 補上 connectivity.ts send/handle + AppState ticker 後即完整 D7）|
...
剩下未 land：**D0** spike（3 spike 結果寫 commit body，hard blocker for D6 + D7 wire-in + D9 wire-in）、**D4** Xcode Watch target、**D5** Watch HK lifecycle、**D6**、**D7 wire-in**、**D8**、**D9 wire-in**、**D10**-**D18**、**D22**-**D27**。D21 視為已落地（label 在 main 上是 `slice-13d/D5`），未來不再 implement D21。D7 + D9 各自的「partial」commit 落了純邏輯子集，wire-in 子集留待 connectivity.ts D3 bridge 落地時補完。

**D-chain 推進策略 update（2026-05-27 evening）**：原規約「forward 接 D4 → D5 → ... 順序進行」翻盤 — 純 TS / 純 logic / 無 native dep 的 commit 可亂序提前 ship（見 2026-05-27 翻盤 ledger row）。**首批落地（2026-05-27 evening session）共 4 + 2 commits**：D20 (`2e3b13d`) + D19 (`791a0ed`) + D7 partial (`9a29ef6`) + D9 partial (`c350b5c`) 四條 pure-logic / partial 模組，加上 D26 partial (`54b8f9d`) ADR doc sweep 與 `faf09f5` 把模式萃成 `ship-partial-pure-logic` project skill。下一個純 TS 候選：**D24 settings readout 邏輯**（last-sync formatter，~80 LoC）。Watch 端（D8 / D10-D17 / D22）+ 卡 D0 spike 的 D6 / D7 wire-in / D9 wire-in native bridge 留實機開工時做。
