...
- `SessionStatsPanel` variant logic 不變 — Phase A 已正確 branch on `is_watch_tracked`、Phase B 只是讓真 Watch-tracked session 自然 trigger 5-tile（dev toggle 移除後條件為 `sessionState.is_watch_tracked === true`）（D5 commit `7b07f9b`：predicate 切換到 v024 `session.is_watch_tracked` column，取代原本預想的 `healthkit_workout_uuid !== null`；以 column 為單一 source 比 HK UUID 更早可用，且不受 HK 寫入時機影響）
