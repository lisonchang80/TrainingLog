...
| NEW-Q47 | D0 spike 內容 | Spike A (Q28 trigger-only watchOS 11+) + Spike B (Q22 paired-share HK) + Spike C (Q5 react-native-watch-connectivity New Arch) 一次跑、3-5 hours 實機。**Spike C 已 land 為 D0 partial (2026-05-27 14:18 真機 PASS 44ms、見 Q5 + shipped 表)**；Spike A/B 仍 pending、卡 D4 Watch target 先建。|
...
剩下未 land：**D0 spike A/B**（spike C 已 land 為 D0 partial、A/B 卡 D4 Watch target）、**D4** Xcode Watch target、**D5** Watch HK lifecycle、**D6**、**D7 wire-in**、**D8**、**D9 wire-in**、**D10**-**D18**、**D22**、**D23**、**D24 wire-in**、**D25**、**D27**。D21 視為已落地（label 在 main 上是 `slice-13d/D5`），未來不再 implement D21。D7 / D9 / D24 各自的「partial」commit 落了純邏輯子集，wire-in 子集留待 connectivity.ts D3 bridge / settings repository 落地時補完。
