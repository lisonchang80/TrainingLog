...
- 必填、default「其他」（既有 65 個 built-in seed 要人工填值 in v013 migration）
...
**Migration v013 內容**（一併在 Q5 migration 內）：
```sql
...
### v013 — Exercise Library v2 columns + muscle naming revise
```sql
-- Equipment
...
-- (撰寫在 src/db/seed/v013ExerciseLibraryEquipment.ts)
```

### v014 — Reusable Superset entity
```sql
CREATE TABLE superset (
  id TEXT PRIMARY KEY NOT NULL,
  name TEXT NOT NULL,
...
3-4 週實作（吸收 17 題 grill + v013/v014 migration + 4 個 amendment + UI 全重寫）
