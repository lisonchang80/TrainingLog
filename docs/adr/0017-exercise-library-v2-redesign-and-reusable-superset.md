...
**媒體灌入 amendment（slice exercise-library-media-import, 2026-06-24）** — Q8 placeholder 庫實際填入策展真人照：

- **來源**：[Free Exercise DB](https://github.com/yuhonas/free-exercise-db)（Public Domain、每動作恰 2 張真人健身房照 `0.jpg` 起 / `1.jpg` 終）。策展 230 動作（**167 有真照 / 63 placeholder**），逐部位校對工作流見 skill `exercise-db-curation` + `docs/exercise-media-import/`。
- **儲存模型翻盤（L98）**：原 Q8 設計 `Documents/exercise_media/{id}` 是給 **user 上傳 override** 的可變檔路徑模型；內建策展照改走**打包進 app**（`assets/exercise-media/{fedb_id}/{0,1}.jpg`、`sips` 縮 ~600px、~19MB）。`media_path` 欄改存 **require-map 的 key**（= Free-Exercise-DB folder id），不是 Documents 路徑；helper `resolveExerciseMedia(key)` → `[require(0.jpg), require(1.jpg)]`（Metro 不能動態 require、map 為靜態字面量 `src/db/seed/exerciseMediaMap.ts`）。Q8 的「user 上傳 override 進 Documents」維持有效但**延後**（v1 只灌內建照、不開上傳）。schema **不動**（`media_path TEXT NULL` 沿用、key 與 path 共用同欄）。
- **卡形（L99 補充）**：動作卡縮圖 **96×96 圓圈 → 16:9 長方形**（`library.tsx` `thumbWrap`）；grid 仍只顯示 frame 0 靜態 poster（167 卡不跑 timer）。
- **詳情頁 2 格交替**：`components/exercise/exercise-media-frames.tsx`（expo-image、`transition` crossfade、900ms 來回切 frame 0/1）模擬 loop GIF；只詳情頁用。
- **庫對齊（grill 2026-06-24）**：策展 230 vs 既有 66 內建 seed 走「**C 折衷**」——24 同名沿用舊 id 補 media、206 策展新動作 INSERT（`exId(67..272)`）、39 既有-only 被細分版取代 `is_archived=1` 封存（FK 安全、舊 session 仍可解析）、3 個策展真的沒有的（山羊挺身/板凳撐體/驢式提踵）保留 active。**淨 active 內建庫 = 233**（migration `v028_exercise_media_library`）。
- **已知缺口（defer）**：(1) 206 新動作只有 muscle_group、**無細部 primary/secondary muscle 連結** → 詳情頁解剖熱區只顯示大肌群；(2) `load_type` 策展只標 13/230，其餘以 `equip='自重'`/`（自重/輔助/負重）`後綴 best-effort 推導，少數無後綴自重動作可能誤判 loaded。
- **i18n**：206 新動作 DB `name` 一律存 zh（placeholder 本來就無英文；全 zh 使 i18n key 為中文＝不撞既有 66 的英文 key、避 TS1117 dup）。**全 230 新動作**都補進 `zh.exercise`(zh→zh identity) + `en.exercise`(zh→英文) 兩 map（維持 shape-invariant parity）：143 真照英文取 FE-DB `en`，63 placeholder 英文取 `build_seed.py` 的 `PLACEHOLDER_EN` 手譯字典（標準健身術語）。**坑**：63 placeholder 起初省略 en.exercise→en locale fallback 吐中文＝動作庫中英混雜（device 抓包），補 `PLACEHOLDER_EN` 後 EN 全英、ZH 全中。
