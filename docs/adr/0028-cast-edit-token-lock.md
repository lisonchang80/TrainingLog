...
| 日期 | 翻盤項 | 原拍板 | 新拍板 | 觸發 | 關聯 commit |
|---|---|---|---|---|---|
| 2026-06-28 | restart 行為 | 鎖/session 只在記憶體（重啟歸零）| **iPhone 持久化 lock state + 重投影復原**（iPhone 重啟 seed、Watch 重啟 handshake `hasLocalSession=false`→iPhone 重投影、Watch-was-holder→翻回 iPhone）| device smoke ①② | （impl pending）|
