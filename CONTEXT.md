...
| 無 Program | **無** | UI 顯短版（DB schema 字串 slice ship 時定）|
...
- **Lifecycle 4 路徑**（per ADR-0019 § Q9）：
  - **Start**：Templates tab → tap Template name → bottom sheet（週期 + 強度 picker，sticky last-selected，首次 fallback「無」）→ `[編輯模板]` / `[開始訓練]`
...
**In-session ⚙️ menu 4 項**（ADR-0019 § Q5）：📝 編輯備註 / ⏱️ 休息秒數 / 🔄 換動作（不可挑 RS）/ 🗑️ 刪除動作（confirm dialog）。「🔗 連結為超級組」第 5 項由 Q7 cluster 來源唯一性拍板移除。
