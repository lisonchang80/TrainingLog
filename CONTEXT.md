...
- **v1**（提前）：WRITE TrainingLog Session 回 HealthKit `HKWorkoutType=traditionalStrengthTraining`（**Slice 13d 修訂**：iPhone 13c `saveTrainingLogWorkout` writer 為唯一 HKWorkout 寫入點；Watch 端 `HKWorkoutSession` 純 trigger-only sampling、end 時 `HKLiveWorkoutBuilder.discardWorkout()` 不寫 entry。原「Watch 端寫入」路徑廢止、見 ADR-0008 § Slice 13d Amendment。**2026-05-26 activityType fix**：原 13c grill 拍 `functionalStrengthTraining`，commit `e5732ac` 改 `traditionalStrengthTraining`，因「重量訓練」更貼合 traditional = barbell/dumbbell weight lifting；functional = HIIT/circuit），iPhone 端 READ 回拿 HKWorkout.UUID + duration + kcal + HR samples 進 `session.healthkit_workout_uuid` / `session.kcal` / `<HRZoneChart samples={data} />`
...
| **display_rank** | set 的**顯示**排序鍵（reorder / 中插），與身分 `ordering` 解耦；Watch 上 wire、iPhone 渲染按 `display_rank ?? ordering` | `set.display_rank REAL`（v025、nullable、backfill=ordering） | ADR-0019 § 2026-06-02 / skill `set-ordering-surfaces` |
...
**Cast / 投影 session**（ADR-0028）：iPhone 持有的 active session 被推到配對 Apple Watch、讓兩端看同一場訓練。**2026-06-28 翻盤**：原「同時雙向編輯」（forward + reverse 常開）廢止，改 **編輯鎖（edit-token）互斥**——詞彙：

- **編輯權 / 編輯令牌 (edit token)**：cast pairing 中唯一可編輯的權；恰一方持有。
- **持有方 (holder)**：持令牌、可編輯、單向把 live-mirror 推給對方。
- **鎖定方 (locked)**：唯讀即時鏡像 + 鎖定蓋層 + **解鎖鈕**（唯一能發起交接的一端）。
- **token epoch (世代號)**：單調遞增；每次成功交接 +1。收到 epoch> 自己 → 降鎖採用（萬用 self-heal / 強制奪取偵測）；< 自己 → 丟棄。
- **交握 (handshake)**：解鎖 = 3 步交握 `lock-request → lock-grant(帶最終 snapshot) → lock-ack`；holder 收 request 後進 **OFFERING**（停編輯 + flush 確保對方已更新）。
- **強制奪取 (force-take)** / **保留鎖定**：對方不可達時 request 逾時的二選一（強制奪取＝單方 +epoch 接手、代價可能遺失對方未 flush 編輯；保留鎖定＝留唯讀）。
- **發起方初握**：iPhone 投影時即 holder、Watch 即 locked。**僅 cast 受鎖**；單機 session 不受影響。詳見 ADR-0028。
...
- **Lifecycle 4 路徑**（per ADR-0019 § Q9）：
  ...
  - **Finish**：Template-based diff-aware（**無 diff** 直接 finish；**有 diff** 跳 3-option：儲存 / 另存 / 否）；Freestyle 永遠跳 2-option（儲存 / 否；「儲存」走 ADR-0014「另存模板」same flow）。Diff scope 涵蓋 set count / reps / weight / is_logged / set_kind / set_ordering / 加刪動作 / 換動作 / cluster / rest_sec；exercise.notes + session.title **不算 diff**。

**In-session stats panel**（ADR-0019 § Q6 / 翻盤 ADR-0012「session 頂層無 stats」）：位置 P1 = timer header 下方；非 Watch 3-tile 1 row（容量 / 動作數 / 訓練時間）；Watch-tracked 5-tile 2 row（row 1: 容量 / 動作數 / 訓練時間；row 2: 心率 BPM + Z1-Z5 區間色 / 大卡）。「Watch-tracked」= v024 `session.is_watch_tracked` column = device 接到 Watch session 開的標記（slice 13d D5 把 5-tile predicate 從 dev_simulate AsyncStorage toggle 切過來，Q19 拍板用此 column 而非 healthkit_workout_uuid 以避開 iPhone-led + finalize HK write 的 false positive）。**「無 chip / 無 AI」維持** — 翻盤範圍僅 stats panel 這層。
...
**新使用者 Onboarding**（ADR-0029）：首次安裝、且無備份可還原時，一支專屬全螢幕多步精靈（鏡射 program-wizard「Step N of M」、本身即引導不疊 coach）引導初始設定，5 步＝**歡迎 → 訓練經驗 → 模式推薦 → 輸入身體數據 → 連結 Apple Health**。

- **觸發**：`app_settings.onboarding_completed`（bool，無 migration，讀不到＝false）。`OnboardingGate` 掛在 `DatabaseProvider`+`AppModeProvider` 內、旗標未設時整頁蓋掉 tab 導覽不進 router history；在 RestoreGate「新建 DB／無備份／已拒還原」分支跑，**還原成功則直接設旗標跳過**（對齊 ADR-0011 restore-bypass）。旗標只在「完成／跳過」時寫，中途崩潰下次重來（無逐步進度持久化）。
- **新手偵測**＝**一次性提問，非領域欄位**：Step2 單一問題「有無重訓經驗」2 選，答案**只**拿來預選 Step3 模式，**不新增 schema**（「新手」在 App 內唯一 consumer 就是模式推薦，存等級是投機／YAGNI）；唯一被持久化的結果 = `app_mode`。
- **模式推薦**＝opt-in 智慧預選：新手→極簡（標「推薦」）、有經驗→計劃，可改，按下一步才寫 `app_mode`（ADR-0026）。
- **身體數據步**＝純手動，重用 `insertBodyMetric`（體重必填、體脂/骨骼肌選填）；「連 HK 自動帶入體重」= backlog（iPhone 端目前完全不讀 body 資料）。
- **連結 Health 語意限定於「寫回」**：文案僅承諾「完成的訓練寫進健康/Fitness + 配 Watch 記 HR/kcal」，**不提「同步身體數據」**（ADR-0008：iPhone 只寫 HKWorkout + 讀 HR/kcal、不讀 body）；重用設定頁 `requestHKAuthorization`+`hkAuthState`，可「稍後」跳過。
- **可跳過 + 設定頁可重跑**（重跑不重置現有設定，兼測試 reset）。**不含通知權限步驟**——App 全無通知功能，翻 ADR-0011 舊註「onboarding 請求 push permission」。
...
跨層 orchestration 集合（`src/services/**`）— 每個 service 串接 repository / adapter / UI 的一條流程。與 `## Domain 模組`（純函式）互補；改 Watch 同步、HealthKit、備份還原時從這裡定位。

**Today tab / Session 流程**

- **`loadTrainingTabState`** (`src/services/loadTrainingTabState.ts`) — Today tab 開機查詢 fan-out + derivation，回傳 `TrainingTabState` 給 `app/(tabs)/index.tsx` 二次映射成 `SessionState`
- **`backfillSession`** (`src/services/backfillSession.ts`) — 補登：`backfillBlankSession` / `backfillSessionFromTemplate`（backfill-sheet 的寫入路徑）
- **`endSessionReconciler`** (`src/services/endSessionReconciler.ts`) — 結束 session 的 ack/reconcile 狀態機（`idle → waiting → acked → reconciled`，5s timeout）
- **`endSnapshotReconcile`** (`src/services/endSnapshotReconcile.ts`) — Watch 端結束時的 end-snapshot 與 iPhone DB 對帳（`reconcileEndSnapshot`）
- **`ensureTemplateVariant`** (`src/services/ensureTemplateVariant.ts`) — start template 前確保 (name, 週期, 強度) variant 存在

**Watch 同步（iPhone 端；WC protocol 本體在 `src/adapters/watch/`）**

- **`watchSessionStart` / `watchSessionEnd` / `watchSessionCast` / `watchSessionDiscard` / `watchSessionResolve`** (`src/services/watchSession*.ts`) — session 生命週期的 push 與 handler（`pushStartToWatch` / `pushEndToWatch` / `pushCastToWatch` / `onDiscardSession` / `onStartResolve`）
- **`iphoneLiveMirrorProducer`** (`src/services/iphoneLiveMirrorProducer.ts`) — iPhone→Watch 反向 live mirror 推送（debounce 280ms；Phase C-core，見 `reverse-sync-apply-surfaces` skill）
- **`watchLiveMirrorReceiver`** (`src/services/watchLiveMirrorReceiver.ts`) — Watch→iPhone live mirror 接收 / `parseLiveMirrorSnapshot`
- **`replaceLiveMirror`** (`src/services/replaceLiveMirror.ts`) — live mirror snapshot → DB session tree 對帳（`reconcileSessionTree`）
- **`watchLiveTicksReceiver`** (`src/services/watchLiveTicksReceiver.ts`) — Watch HR/kcal live ticks 接收 + projection
- **`freestyleFirstAddPush`** (`src/services/freestyleFirstAddPush.ts`) — freestyle session 加入第一個動作時是否觸發 cast push（`shouldFireFirstAddPush`）
- **`watchSyncReadout`** (`src/services/watchSyncReadout.ts`) — 設定頁 Watch 同步狀態 readout
- **`watchHandlerResult`** (`src/services/watchHandlerResult.ts`) — WC handler 共用 result 型別（handler never-throws invariant 的載體）
- **`castLockPersistence`** (`src/services/castLockPersistence.ts`) — cast-edit token lock 持久化（ADR-0028，key `cast_lock_state`）

**HealthKit / 備份還原**

- **`healthkitSessionSync`** (`src/services/healthkitSessionSync.ts`) — `syncSessionWithHealthKit`（HKWorkout 寫入）+ `rehealSessionKcal`
- **`backupService`** (`src/services/backupService.ts`) — iCloud Drive 全庫備份 runner（ADR-0011）
- **`restoreService`** (`src/services/restoreService.ts`) — 備份還原（boot 時經 `components/restore-gate.tsx`）
- **`restoreDepsWiring`** (`src/services/restoreDepsWiring.ts`) — restore 依賴注入 wiring（iCloud module ↔ file ops）
- **`jsonExport`** (`src/services/jsonExport.ts`) — 全庫 JSON 匯出（format v1）
...
- **Q10 Multi-device 策略 + Watch v1 範圍**（ADR-0008 已釘）：
  ...
  - ✅ **Q10.1.1** Watch 角色 = α 簡易（完整 Session UI 但功能受限）+ c (HKWorkoutSession 心率+卡路里+workout 寫入)（**Slice 13d 修訂**：HKWorkoutSession 改為 trigger-only sampling、不寫 workout entry；iPhone 13c writer 為唯一寫入點。見 L573-586）
  - ✅ **Q10.2** 主從關係 = α（Watch 主），訓練全程 iPhone 不掏（i 主：iPhone 在包包/置物櫃 BLE 範圍；ii 偶爾：留車上 / 家裡）（**Slice 13d 修訂**：改為**雙向 initiator**，兩端 UI 都有 [開始訓練] 入口、user 隨手挑哪端 initiate。見 L573-586）
  ...
  - ✅ **Pre-session vs in-session 兩態**（**Slice 13d 修訂**：原子三步 ①②③ 在雙向 initiator 後變為 cross-device — Session row 創建移到 iPhone (SQLite SoT)、HKWorkoutSession.start() 在 Watch 端（若 Watch-tracked）；bw_snapshot 鎖定改為 session-start moment。見 L573-586）：
  ...
  - ✅ **HealthKit 整合**（v1 提前）：HKWorkoutType=`traditionalStrengthTraining`；Watch 寫 metadata duration/卡路里/平均+max HR + custom `trainingLogSessionUUID`；iPhone READ 4 metric + HKWorkout.UUID 進 `session.healthkit_workout_uuid`（**Slice 13d 修訂**：Watch 不再寫 metadata、不寫 HKWorkout；iPhone 13c writer 為唯一 HKWorkout 寫入點、metadata 由 iPhone 端組裝。見 L573-586 + ADR-0008 § Slice 13d Amendment）
  ...
  - **Slice 13d 擴張 (2026-05-26)**：α-model 完整實作 grill，多項翻盤（見 ADR-0008 § Slice 13d Amendment + ADR-0019 § Slice 13d Amendment 完整 grill table）：
    - 「全程 Watch 主、iPhone 不掏」→ **雙向 initiator**（兩端 UI 都有 [開始訓練] 入口、SQLite 寫 iPhone、另一端 WC push live mirror）
    - 「Watch 寫 HKWorkout」→ **trigger-only sampling**（Watch `HKWorkoutSession` 開 HR/kcal sampling 但 end 時 `discardWorkout()` 不寫；iPhone 13c writer 為唯一 HKWorkout entry；dual-write race 消失）
    - 「bw_snapshot pre-session 鎖定」→ **session-start moment 鎖定**（雙向 initiator 後 iPhone 必在 start moment reachable）
    - 「Watch 寫 HKWorkout metadata」→ 移除（iPhone 13c writer 處理 metadata）
    - 「FEATURE_WATCH_HANDOFF gate」→ **移除按鈕、flag 退役**（α-model native handoff 取代）
    - 新增 schema column：`session.is_watch_tracked INTEGER DEFAULT 0`（v024）作 5-tile predicate；取代原 dev_simulate_watch_tracked toggle（13d D2 fail-stop 砍）
    - 新增 WC mapping 13 message kind（handshake / start-from-watch / start-from-iphone / set-completed / set-modified / set-deleted / set-added / exercise-added / exercise-deleted / hr-tick / kcal-tick / end-session / settings-sync）；set-modified 採 diff + per-field LWW ts (in-memory map)（**2026-06-01 修訂**：實為 **17 kind** — 補 `start-reconcile`(NEW-Q50) / `start-resolve`+`discard-session`(D31) / `live-mirror`(sync fast lane)；NEW-Q50 已砍 per-field LWW，live mirror 改整包 snapshot replace + 單調 `rev` 反序守門，見 ADR-0019 § 2026-06-01）
    - 新增 iPhone Live Activity（ActivityKit、鎖屏 + Dynamic Island）
    - 新增 Watch picker root 三步（[計劃訓練 一鍵] + [模板訓練 dropdown] → 模板 name → 計劃方塊 → 強度方塊）
    - 新增 Watch in-session UI（vertical scroll set logger + 進度條 + Active row highlight + Digital Crown 滾 weight/reps + tap ✓ auto-advance + inline nested cluster + ⋯ menu 4 項 + ⚙ settings 5 項）
    - Watch 端 user 主動限制：**不能 +動作**（畫面小、選 exercise 麻煩）；要 +動作 → iPhone +動作、Watch via WC sync 自然看到
    - pause 紀律維持（Q9 不翻盤）— Watch / iPhone 都不顯 pause button
    - D0 spike 驗 3 假設（trigger-only watchOS 11+、paired-share HK permission、react-native-watch-connectivity New Arch 相容），結果決定 fallback path 啟動與否
- **Q11 HealthKit 整合邊界（剩餘部分）**：
  - ~~v1：Apple Health cardio workouts READ~~ → **延 v1.5+**（先 ship v1 再做）
  - ~~v1.5+：TrainingLog Session WRITE 回 HealthKit~~ → **已提前到 v1**（Q10 / ADR-0008，由 Watch 端 HKWorkoutSession 寫入）（**Slice 13d 修訂**：改為 iPhone 13c `saveTrainingLogWorkout` writer 為唯一 HKWorkout 寫入點；Watch HKWorkoutSession 純 trigger-only sampling、`discardWorkout()` 不寫 entry。見 L573-586）
  - **READ** v2+：bodyweight、HRV、睡眠等 body data
  - HealthKit Permission UX（v1）：app 第一次啟動時系統 dialog 請求 `HKWorkoutType` + `HKQuantityTypeIdentifier.heartRate` + `HKQuantityTypeIdentifier.activeEnergyBurned` 三個 scope（**Slice 13b Q4 修訂**：原計畫 2 scope，13b 確定為 3 scope 含 activeEnergyBurned）
...
- **Q15 Set logger UI redesign**（grill 主結構完，已寫入 `docs/adr/0012-set-logger-redesign-schema-and-affordances.md`）：
  ...
  - ✅ **Q15.5 Per-row 結構 + Affordance + Dropset Cluster + C3.1 拍板**（2026-05-11；本輪 grill 修訂 Q15.1 / Q15.2 / Q15.3 / Q15.4 多處，整合 reference UI 競品 dropset 模塊）：
    ...
    - **Schema 變動（v008 累加 — 含 Q15.2 / Q15.3 既拍）**：
      ...
      - **新增** `set.ordering INTEGER NOT NULL`（顯式排序，migration 依 created_at 補值；實際欄名 `ordering`，早期 doc 誤記 `position`，2026-06-02 對齊）
      - **新增** `set.parent_set_id TEXT NULL REFERENCES set(id) ON DELETE CASCADE`（dropset cluster B3）
      - **v025 修訂（2026-06-02）**：新增 `set.display_rank REAL`（nullable、backfill = `ordering`）作 Watch reorder / 中插的**顯示**排序鍵；身分仍 `(session_exercise_id, ordering)` 不變。有 6 條獨立 render 排序路徑都改 sort by `display_rank ?? ordering`（權威清單見 skill `set-ordering-surfaces`）。
  ...
  - ✅ **Q15.5b Cluster 級 affordance + C3.2 + F + G 拍板**（2026-05-11；Q15.5 leftover 一次清掉，Q15 set logger redesign 主結構全完）：
    - **Q15.5b Cluster 首 step row 三 gesture（全 α）**：
      ...
      - **長按** → 整 cluster 浮起拖移：children 跟 root 一起移動，cluster 是 reorder 單位（schema 上 parent_set_id 不變、只 ordering 重編）
