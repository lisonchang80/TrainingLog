...
- **v1**（提前）：WRITE TrainingLog Session 回 HealthKit `HKWorkoutType=functionalStrengthTraining`（**Slice 13d 修訂**：iPhone 13c `saveTrainingLogWorkout` writer 為唯一 HKWorkout 寫入點；Watch 端 `HKWorkoutSession` 純 trigger-only sampling、end 時 `HKLiveWorkoutBuilder.discardWorkout()` 不寫 entry。原「Watch 端寫入」路徑廢止、見 ADR-0008 § Slice 13d Amendment），iPhone 端 READ 回拿 HKWorkout.UUID + duration + kcal + HR samples 進 `session.healthkit_workout_uuid` / `session.kcal` / `<HRZoneChart samples={data} />`
