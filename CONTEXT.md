...
- **Q10 Multi-device 策略 + Watch v1 範圍**（ADR-0008 已釘）：
  ...
  - **Slice 13d 擴張 (2026-05-26)**：α-model 完整實作 grill，多項翻盤（見 ADR-0008 § Slice 13d Amendment + ADR-0019 § Slice 13d Amendment 完整 grill table）：
    ...
    - 新增 WC mapping 13 message kind（handshake / start-from-watch / start-from-iphone / set-completed / set-modified / set-deleted / set-added / exercise-added / exercise-deleted / hr-tick / kcal-tick / end-session / settings-sync）；set-modified 採 diff + per-field LWW ts (in-memory map)
