...
export default function TodayScreen() {
  ...
  // refreshRef bridges the WC listeners (extracted to useWatchSync) to the
  // freshest `refresh` closure without forcing the effect to re-subscribe
  // on every render.
  const refreshRef = useRef<(() => void) | null>(null);
  refreshRef.current = refresh;

  // Watch⇄iPhone live-sync listener cluster — end-session dual-channel +
  // the handshake / start-from-watch / start-resolve / discard / live-
  // mirror / hr-kcal-tick listeners — extracted to a hook (report 09 #2).
  // It owns the useEffect subscription wiring + cleanup; per-message
  // handling lives in src/services/watch*. ⚠️ device-smoke-gated (touches
  // live Watch sync) — not a blind merge.
  useWatchSync(db, {
    refreshRef,
