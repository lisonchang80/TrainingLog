...
2. Watch build 加旗標，否則 sim 走 mock picker（`ContentView.swift` 的
   ...
   ```
3. iPhone 端：從待測分支冷建 dev-client（帶該分支的 WC 原生模組）：
   ```bash
   cd <worktree>/ios && LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 pod install
   cd <worktree>     && LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 \
     npx expo run:ios --device <phone-sim-udid>   # 建+裝+啟+Metro，~15-20min
   ```
   ⚠️ **`pod install` 一定要帶 `LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8`**——CocoaPods 1.16.2 + Ruby
   4.0.3 在無 UTF-8 locale 的 shell 會炸 `Encoding::CompatibilityError: Unicode Normalization not
   appropriate for ASCII-8BIT`（把 Podfile 路徑當 binary 編碼、`installation_root` 崩）；且 `| tail`
   會吞掉 pod 的非零 exit（tail 自己 exit 0＝假成功）→ **看 `ios/Pods/` 是否真生成才算數**。`expo
   run:ios` 那行同理帶 locale。（2026-07-05 整合分支冷建實證。）
...
- ⭐**7006 復原：改用「app-relaunch」而非「sim-reboot reset」**（2026-07-05 整合分支 smoke 實證）。
  `sim-wc-smoke-env.sh reset` 會重開兩台 sim——但 **sim-reboot 本身正是一直洗壞 wcd 對端登記的元兇**
  （跑 2 次 reset、7006 仍 ~88 hits/3min 死）。更溫和且有效＝**只重啟兩端 app（不重開 sim）、錶先啟機後啟**：
  ```bash
  xcrun simctl terminate "$PHONE" com.lisonchang.TrainingLog 2>/dev/null; xcrun simctl terminate "$WATCH" com.lisonchang.TrainingLog.watchkitapp 2>/dev/null
  xcrun simctl launch "$WATCH" com.lisonchang.TrainingLog.watchkitapp; sleep 8
  xcrun simctl launch "$PHONE" com.lisonchang.TrainingLog
  ```
  **fresh phone process 讓 interactive `replyHandler` 車道全恢復**（7006 從 88→2、handshake/start/end/live-mirror 全通）。
- ⭐**7006 下「兩條車道分野」**：7006 只擋 `transferUserInfo`（durable/TUI 車道）；**interactive
  `sendMessage`+`replyHandler` 車道 fresh process 後可獨立恢復**。故即使 7006 未清，**watch→phone 全鏈仍可 smoke**
  （handshake 回覆走 replyHandler → 真模板列表能到、Watch-led start/end/live-mirror 全驗得了）；被擋死的只有走
  durable 車道的 **phone→watch cast / unlock 回覆**（→ 留實機）。判讀＝先 app-relaunch + 真流量（點模板看列表到不到），別急著怪 code。
- **DB 驗證路徑**：sim app 的 DB 在 `$(xcrun simctl get_app_container <phone> com.lisonchang.TrainingLog data)/Documents/SQLite/traininglog.db`；
  C-id idTree 採納看 session id 有 **`W-` 前綴**（Watch 鑄的 id 逐字沿用）+ 無 dup `session_exercise`；set 表欄名是 `weight_kg`（非 `weight`）。
