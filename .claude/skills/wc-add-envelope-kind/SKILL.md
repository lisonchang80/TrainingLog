...
- **Timeout = the WC framework's own ~5s reply timeout** (errorHandler → resume nil → error state). Do NOT hand-roll a shorter watchdog with a shared `resumed` flag across the `replyHandler`/`errorHandler` closures — that's a `Sendable`-captured-`var` double-resume hazard. `withCheckedContinuation` + Apple's single-handler contract (exactly one of reply/error fires) already guarantees the spinner resolves; the coordinator just mirrors `requestHandshake` verbatim.
...
- Validated 2026-06-09/10 (`history-request`, #311-A, **shipped on `slice/13d-watch-history-pull`, device path-1/3 ✅**): pure builder + handler + mount + 13 tests; Swift `ExerciseHistoryView` 4-state + 手機-aligned card (topSet line + numbered rows) + coordinator `requestExerciseHistory`; injected `load:` closure; nav-title-short fixes a long-name back-chevron loss.
