...
- **`devicectl device process launch` GENUINELY FAILING (`Code 10002` / `FBSOpenApplicationServiceErrorDomain error 1`, "The request to open … failed") is NOT a blocker — the `install` already landed the bundle.** Distinct from the benign 1002 warning above: here the launch truly doesn't happen (no `Launched application` line). This device just doesn't reliably cold-launch via devicectl (seen 2×, 2026-07-04). Fix: **tell the user to open TrainingLog from the iPhone home screen** — a manual cold-start also re-fetches the fresh JS bundle from Metro (which a devicectl launch may skip), so it's the better path anyway when a JS-side change needs to load. Don't retry the launch or treat it as an install failure; confirm the install with `devicectl device info apps … | grep -i training` and hand off the manual open.
