...
| D26 partial 四次補刀 `5553c59` | ADR-0019 add D5 native / D6 / D7-TS rows, retroactive drift annotation on D7-partial row (superseded by D7-TS sendMessage timeoutMs path), narrow remaining-未-land (移除 D5 / D6 / D7 wire-in、加 D7-Swift on-branch row), update header timestamp evening → night | doc-only |

**Doc-sweep cadence** — 4 ADR sweeps + 2 skill self-updates landed alongside 8 code commits across two sessions (2026-05-27 evening + night). The pattern: every 2-3 ship-partial code commits → one `docs(slice-13d): D26 partial 補刀` to refresh the shipped table + remaining-未-land list + 翻盤 ledger. Don't let doc drift accumulate across an evening — three small sweeps cost less attention than one big end-of-session reconciliation, and future-you reading the ADR mid-session sees current state without guessing what's "really" landed.

**Drift annotation on superseded past rows (added 四次補刀 2026-05-27 night)**: when a later grill round overturns a *past partial* commit's wire-in plan, don't just leave the old row pristine — readers landing on it via grep / TOC will assume the wire-in is still pending. Inline-annotate the old 備註 cell with a forward-pointing note: `**註**：<superseder commit> grill 拍板用 <new approach> 路徑、未 wire 此模組；retained as future generic primitive、不再算 <D-slot> critical path`. Example from 2026-05-27 night sweep: D7-partial reducer (`9a29ef6`) row got `**註**：D7-TS (cc3307d) grill 拍板用 sendMessage timeoutMs Promise-based 路徑、未 wire 此 reducer；此模組 retained as future generic primitive (需 AppState ticker 的場景才會用)、不再算 D7 critical path 一部分`. Same convention as grill-with-docs's "Mark revisions explicitly when later rounds overturn earlier decisions" — applied to ADR shipped table instead of grill decisions.
