...
**A. ClipPath partition — vertical SPLIT_X** (best for collapsed slugs with horizontal partition):
- When two sub-divisions should EXACTLY tile the parent slug (no gaps, no overflow) AND the divider runs vertically
...
**A2. ClipPath partition — diagonal split via coord-picker** (when fiber direction is angled):
- For muscles whose sub-heads run DIAGONALLY (bicep long/short, possibly trapezius later) a vertical SPLIT_X mis-aligns with anatomy
- Ask user to click 2 keypoints per side on coord-picker (top + bottom of diagonal). Mirror about user-derived axis (sum check)
- Extrapolate the diagonal line beyond bbox (top y_buffer above, bottom y_buffer below) so the resulting half-polygons fully contain the slug bbox before clipping
- Render two `<Path d="M...L...L...L...Z">` half-polygons per arm, each clipped to the same `<ClipPath>` wrapping the package slug
- **Anti-alias safety**: lateral + medial halves MUST share BYTE-IDENTICAL diagonal edge coordinates — copy-paste, never re-compute on the other side. Otherwise sub-pixel mismatch shows as a seam.
- Pattern used: bicep round 2 (2026-05-24) — `PATH_BICEP_L_LATERAL_HALF` / `_MEDIAL_HALF` per arm
...
- ❌ Pick vertical SPLIT_X for muscles whose fibers run diagonally (bicep long/short). User's anatomy intuition will flag it. Use Pattern A2 (diagonal split via coord-picker).
- ❌ Forget that package's `defaultStrokeWidth=0` skips per-slug borders. Body's outer border is one path that omits the head — if a head/hair slug needs a stroke, paint the slug path again with `fill="none" stroke=<colour> strokeWidth=2 vectorEffect="non-scaling-stroke"` as a stroke-only overlay layer. Without `vectorEffect`, the viewBox-space strokeWidth scales down to sub-pixel at small BODY_SCALE.
- ❌ Apply Pattern C extension before checking if the package's source paths ALREADY convey the structure. Some package slugs render as multiple sub-paths with white gaps that imply anatomy (e.g. triceps back slug = 3 sub-paths suggesting lateral/medial/long heads). Adding Pattern C strokes ON TOP competes with the gaps instead of reinforcing them. **Always inspect the package's raw path data BEFORE deciding Pattern C is needed** — count sub-paths first. (Agent B 2026-05-24 ABANDONED triceps Pattern C overnight after self-eval PNG showed exactly this competing-noise effect.)
- ❌ Relitigate a partition pattern (e.g. vertical SPLIT_X) after a coord-picker-validated alternative (e.g. diagonal A2) already shipped, without quantifying the per-y drift first. Diagonal-fiber muscles (bicep: 19.4u mid-x drift across 86u belly height = 47% of belly width) make vertical impossible to align uniformly. **Quantify drift before reopening the question** — if drift > 20% of partition width, the alternative is mathematically inferior, not stylistically. (Agent A 2026-05-24 confirmed A2 wins on every axis for bicep; B variants kept behind a single-constant `BICEP_PATTERN` toggle as a research artifact only.)
...
## Historical example session 4 — bicep diagonal split + head outline (2026-05-24)

2 rounds for biceps + 1 mini-pattern (head outline), 3 commits, no agent spawn:

- **Round 1 (commit 26ba1a4)** — Pattern A vertical SPLIT_X attempt. Decoded `PACKAGE_BICEP_L/R` bbox via offline Bezier sampling script (Node, in /tmp), picked horizontal midpoint as SPLIT_X. Wired up like deltoid.
- **Round 2 (commit 188d723)** — User feedback via `bicep-coord-picker.html` (built on session 2's template, swap viewBox to `160 390 410 140` covering both biceps + dashed overlay of current SPLIT_X for reference). User clicked 4 keypoints: 2 per arm (top + bottom of diagonal). Mirror about x=364 confirmed (728-x reflection passes for both endpoints). Promoted to **Pattern A2 diagonal**: extrapolated diagonal to y=395..510 buffer, replaced 2 Rect per arm with 2 trapezoid Path per arm, both clipped to PACKAGE bicep ClipPath. Shared identical diagonal edge coords between lateral + medial halves (anti-alias safety).
- **Head outline mini-pattern (commit 0734b6f)** — Discovered package's body border `<G strokeWidth=2>` path starts at `M 309.48 168.91` (below jaw), skipping head + hair. Slugs themselves have no stroke (`defaultStrokeWidth=0` and our `defaultStroke=#9CA3AF` only sets colour). Fix: copy package's `head` + `hair` slug paths verbatim into `PATH_HEAD_OUTLINE_FRONT/BACK`, render as one Path per side with `fill="none" stroke=COLOR_OUTLINE strokeWidth=2 vectorEffect="non-scaling-stroke"` at end of Front/Back overlay. `vectorEffect` was essential — without it, the viewBox-space stroke 2 renders at sub-pixel width at `scale=0.5` and stays invisible.

**Key takeaway**: when adding a NEW partition kind (diagonal vs vertical), the coord-picker HTML pays for itself in 1 round. Build the picker as soon as user shows a 2-keypoint or 4-keypoint mental model — don't try to guess slope from the first feedback. Also: package slug stroke gaps are silent; the body LOOKS normal until user squints and asks "where's the top of the head" — keep a checklist of `defaultStrokeWidth=0` per-slug stroke caveats when adopting new packages.

## Historical example session 5 — overnight 3-agent anatomy parallel (2026-05-24)

3 anatomy agents in parallel (biceps Pattern B revisit / triceps Pattern C / abs Pattern C), each producing reference PNG via headless puppeteer + self-eval markdown. Integrated next morning via cherry-pick; zero conflicts because each agent worked on a distinct anatomical region with append-only edits to `body-overlay-paths.ts` + `muscle-body-tagger.tsx` + `body-heatmap.tsx`.

- **Agent A — biceps Pattern B vertical revisit**: built 4 SPLIT_X candidates (bbox-mid / mass-center / medial-5 / lateral-5), rendered all + the shipped A2 diagonal via puppeteer. Self-eval quantified per-y mid-x drift = 19.4u across 86u belly height. Verdict: A2 wins on every axis. B variants kept behind `BICEP_PATTERN: 'A2'` toggle constant — single-line flip to revisit, no re-coding. 3 commits, 1393 tests green.
- **Agent B — triceps Pattern C horseshoe**: implemented 4 cubic Bezier strokes per arm pair (lateral/medial/long head suggestions), rendered primary/secondary/inactive/zoom PNGs. Self-eval: package's back-view triceps slug ALREADY renders as 3 sub-paths with white gaps implying head split — Pattern C strokes competed with the natural gaps, reading as noise. Verdict: ABANDON. Cherry-pick only the harness + findings + PNGs commit; skip impl commit.
- **Agent C — abs Pattern C linea alba + 6-pack**: 4 stroke paths (vertical linea alba M364 432→708 + 3 horizontal tendinous intersections y=500/534/581). Sampled linea alba x=364 via midline of L_max/R_min sub-paths (not naïve viewBox center 362). All strokes `vectorEffect="non-scaling-stroke"` (essential at BODY_SCALE=0.5). Verdict: SHIP, confidence 4/5. Obliques skipped (package's oblique slug already has 8 sub-paths per side approximating fiber direction — would compete same as triceps lesson).

**Key takeaways**:
- **Self-eval PNG is the right gate for anatomy iteration overnight**. Each agent decides ship/abandon based on visual output, not just "does the code compile". `puppeteer` is non-expo so `npm install --save-dev puppeteer` is safe in any worktree.
- **A 4-agent parallel pattern works for anatomy when regions are disjoint**: append-only edits to shared `body-overlay-paths.ts` merge clean because each agent adds new const names with anatomy prefix. Cross-agent conflict probability is ~zero.
- **Cherry-pick selectively when an agent recommends abandon**: keep the findings + harness commit (documentation value), skip the impl commit (would ship the rejected pattern).
