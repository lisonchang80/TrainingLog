...
- ❌ **用戶口語短指令有正反兩種合理解讀時直接 implement** → 2026-05-20 dropset polish wave 兩次猜錯方向：(a)「隱藏為打勾」我做「隱藏已打勾」、用戶 push back「應該是隱藏未打勾」；(b)「session 遞減組補上 -/+」+「+1 swipe 異常」我做「extend chain」、用戶後來說「應該是新 D2 cluster」。兩個都是用戶簡短指令省略主語/方向，正反邏輯都通。**成本**：每次方向錯 = 反向 implement + commit + push + 用戶 reload 截圖 → 反饋（~15-30 min round-trip）。**規則**：簡短指令 (≤10 字) 觸及 (a) 開關 ON/OFF 方向、(b) 加減動作方向、(c) cascade 範圍 → **prompt 內先 confirm「我理解的是 X，例如 Y 場景下會 Z；意思對嗎？」**再 implement。30 秒 grill 比 30 分鐘重做便宜。Prompt 範例：「我理解你要 hide checked sets（is_logged=1 隱藏）、剩沒 check 的列；對嗎？」用戶 “對” 或 “反過來” → 一句話定向。
- ❌ **Dropset chain 結構操作（add/delete/cycle）沒保 contiguity** → 2026-05-20 多輪 fix 揭露：cluster head 砍時 followers orphan（cascade 缺）、cycle warmup→dropset 時 follower 塞 maxOrdering+1（chain 切斷）、+1 swipe 沿用 source set_kind 但 parent_set_id=null（chain 中插假 head）。三個都導致 `computeSessionSetLayout` 把 follower 視為 orphan、UI 出現空 label / 多餘 row。**規則**：任何動到 dropset (set_kind='dropset') 列的 op、寫之前要明列「對 chain contiguity 的影響」: (a) ordering 是否仍 head→followers 連續、(b) parent_set_id 是否仍 valid、(c) cascade 範圍是否含全 chain。**Test heuristic**：dropset chain 改寫後 grep `computeSessionSetLayout` 跟 caller，看 orphan branch（line 140 "Non-dropset row, OR orphan follower"）會不會打到。打到就是結構壞了。
- ❌ **Dropset chain 讀路徑（filter / count / display）沒 chain-aware** → 2026-05-20 wave 12 連續 5 個 bug 同源：(a) `prefillSessionExerciseFromLastSession` SQL `WHERE is_logged = 1` 排除 follower（DB 永遠 0）、(b) `prefill` parent_set_id=null on copy 把 chain 拆成 N 個 orphan head、(c) `computeExerciseProgress.workingDone` 只算 working 不含 dropset HEAD、進度條卡 0/N、(d) template editor solo `workings = s.kind !== 'warmup'` 把整 chain 每 row 都計入「組」、(e) history read mode `computeHistorySetLabels` 每 row 標 D{N}。**規則**：任何讀 set list 的 code path（SQL filter、count helper、UI render）要先回答：「dropset follower 該被當作 head 的鏡像 / 不獨立計數嗎？」答案 ≈ 永遠是 yes。**Trigger**：寫 SQL `WHERE is_logged = 1`、寫 `sets.filter(s => s.set_kind === ...).length`、寫 label / count / volume helper 時、先讀 `.claude/skills/dropset-chain-semantics/SKILL.md` 的 4 invariants + checklist。
- ❌ **跳過 runtime WARN message 直接推測 root cause** → 2026-05-22 wave 18g 「+新建」 從 modal wizard 推 editor 觸發 Maximum-update-depth crash。第一次 attempt 修 setState-in-updater anti-pattern (commit `852fead`) 沒解；用戶 reload 一樣 crash + 貼出**完整 log** 才看見 `WARN: Dynamically changing header's visibility in modals will result in remounting the screen` — 那條 WARN literal 點出了 expo-router 真正爆炸的 API path。**規則**：用戶報 runtime crash 時，**先要 full log（含 WARN messages、不只 ERROR）**再推測。WARN often precedes ERROR by milliseconds 且 message 已經 narrow 到具體 API。Cost saved: 一次 partial-fix commit。
- ❌ **Component-body 內 inline `<Stack.Screen options={{...}}>` 推 modal-presented parent context** → expo-router 在 modal context 偵測「動態改 header visibility」會觸發 remount → setState 跑 effect → 又 setOptions → 又 remount → 無限循環。**規則**：route file 若可能從 `presentation: 'modal'` 的 parent 被 `router.push`，**所有 Stack.Screen options 必須在 `app/_layout.tsx` 靜態註冊**、不在 component body 動態設。Trigger：grep `<Stack.Screen options=` 在 component file 內、檢查該 route 在 `_layout.tsx` 是否已註冊；沒註冊 + 從 modal entry → 必雷。Wave 18g `template/[id]` 修法：layout 新 `<Stack.Screen name="template/[id]" options={{ headerShown: false }} />`、component 內 3 處 inline 全砍。Commit `ce3ca5a`。**已知 backlog**：`app/session/[id].tsx:1603` 同 pattern、目前沒從 modal 進入故沒踩、預防性修可放下波。
...
- `dropset-chain-semantics` (project) — 動到 set filter / count / display / SQL 前先讀 4 invariants + checklist
