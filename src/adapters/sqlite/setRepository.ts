 ...
 * session so each session counts from 1. UUID is REQUIRED (no default) —
 * Hermes lacks a global `crypto.randomUUID`, so the caller must inject
 * (production passes `randomUUID` from `expo-crypto`; tests pass a stub).
 */
